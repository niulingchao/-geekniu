...
const inner_optration_time_filter_warning = require(process.cwd() + "/middleware/filter/operation_time_filter.js").inner_optration_time_filter_warning
...
router.post('/zhmd/device_warning/display_device/check_hearbeat.json', async(req, res, next) => {
    ...
    try {
        ...
        if (current_hour > 12 && current_hour % 2 == 1) { // 如果当前时间是12点之后  两个小时上报一次
        ...
        Promise.map(offline_display_device, async(device) => {
            ...
            if (shop && Object.keys(shop).length > 0) {
                ...
                if (await shop_is_in_warning_time(shop)) {

                    // 查询用户
                    //
                    let user = await User.findOne({ "$and": [{ "user_data.user_group": 0 }, { "user_data.shop_id": shop.id }, { "user_data.is_warning": { "$ne": 0 } }] })
...
function is_current_time_in_warning_during() {
    ...
    if ((current_hour >= 0 && current_hour <= 8) || (current_hour >= 21 && current_hour < 24)) {
...
const shop_is_in_warning_time = async(shop_id) => {
    ...
    return !await inner_optration_time_filter_warning(shop_id);
