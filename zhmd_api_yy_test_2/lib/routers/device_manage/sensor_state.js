...
async function find_show_case_id_of_plans(shop_id, shop_type) {
    ...
    try {
        ...
        let plans = await modle_util.find_docs_form_Data('plan', { "data.plan_type": { "$ne": 10 }, "data.shop_id": shop_id, "data.state": { "$ne": -1 } })
...
function is_sensor_online(sensorheat_time) {
    ...
    //  传感器心跳  正常值为 3 个小时
    if (current_time - heartbeat_time > 3600 * 1000 * 3) return false;
