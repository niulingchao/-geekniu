...
const plan_heat_value_clone = async(req,res)=>{
    try{
        const {heat_plan_id,start_time,end_time} = req.body;
        if (!heat_plan_id) return errorRequest(req,res,'plan_id不能为空');
        if (!start_time) return errorRequest(req,res,'start_time不能为空');
        ...
        const get_int_count = async(plan_ids)=>{
            const options = {
                method: 'POST',
                uri: `${analysis_calculate_url}`,
                body: {
                    conditions:{"$and":[{"event_attr.plan_id":{"$in":plan_ids}},{"$or":[{"event_attr.sensor_type":1},{"event_attr.report_data_type":2}]}]},
                    batch:[{"result_key":"value","cal_param":"id","cal_function":"count"}],
                    start_time,
                    end_time,
                    schema_type:'event',
        ...
        const tasks = sub_plan_ids.map(plan_ids=>{
            return get_int_count(plan_ids);
        });

        let int_count_result = await Promise.all(tasks);
        int_count_result = int_count_result.map(v=>{
            return v.result.pop();
        });
...
router.post('/zhmd_api/shop_plan/heat_value.json',plan_heat_value_clone);
